...
started = {}
tour = []
tour_length = 1000000000000000000000000000000000000000000000000000000
start = 0
    
for i in range(0,num_cities):
    started[i] = "no"
while start < num_cities:
    temp_tour_length = 0
    for i in range(0,num_cities):
        tracker[i] = "unvisited"
    temp_tour = []
    u=start
    while "unvisited" in tracker.values():   
        minimum = 100000
        for i in range(0, len(distance_matrix[u])):
            if tracker[i] == "unvisited" and distance_matrix[u][i] < minimum and distance_matrix[u][i] != 0:
                current = i
                minimum = distance_matrix[u][i]
        temp_tour.append(u)
        tracker[u] = "visited"
        u = current
        started[start] = "yes"
    temp_tour.append(temp_tour[0])
    for j in range(0, len(temp_tour)-1):
        temp_tour_length = temp_tour_length + distance_matrix[temp_tour[j]][temp_tour[j+1]]
    if temp_tour_length < tour_length:
        tour_length = temp_tour_length
        tour = temp_tour[:]
    start+=1
