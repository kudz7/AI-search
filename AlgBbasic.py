...
#this heuristic is simply the distance to the nearest unvisited city from that city being evaluated.
...
def h(j):
    ...
    for i in range(0, len(t)-1):
        distance = distance + distance_matrix[t[i]][t[i+1]]
    return distance
...
while len(open_list) > 0:
    minimum = 10000000000
    for i in open_list:
        #pick the node with the smallfest f-value in the open list
        if f[i] < minimum:
            minimum = f[i]
            z = i
    open_list.remove(z)
    successors = []
    for i in range(0, len(distance_matrix[z])):
        #adding all the children of z to the fringe
        if i != z:
            successors.append(i)
    eval_min = 10000000000
    for j in successors:
        #evaluating a new f(z) for each node on the fringe
        evaluation = f[z]+distance_matrix[z][j]+h(j)
        f[j] = evaluation
        if j not in tour and j not in open_list:
            open_list.append(j)
    #after the current node has been expanded, add it to the tour
    tour.append(z)
tour.append(0)

for j in range(0, len(tour)-1):
    tour_length = tour_length + distance_matrix[tour[j]][tour[j+1]]
